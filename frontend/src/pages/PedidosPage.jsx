  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h2>Pedidos {usuario.rol !== 'admin' ? '(Mis pedidos)' : '(Todos)'}</h2>
      ...
      <div className="card">
        ...
        {cargando ? (
          ...
          <p style={{ textAlign: 'center', color: '#666', padding: '2rem' }}>No hay pedidos para mostrar.</p>
        ) : (
          <>
            <table>
              <thead>
                ...
                {pedidos.map((p) => (
                  <tr key={p.id}>
                    ...
                    <td style={{ display: 'flex', gap: '0.3rem', flexWrap: 'wrap' }}>
                      <Link to={`/pedidos/${p.id}`} className="btn btn-secondary btn-sm">Ver</Link>
